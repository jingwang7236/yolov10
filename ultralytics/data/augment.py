...
def v10_simple_transforms(dataset, imgsz, hyp, stretch=False):
    """Convert images to a size suitable for YOLOv10 training.
    Do not modify the object boxes size.
    防止出现裁切之后的Box没有人的特征的情况，也是尽可能模拟测试
    """
    pre_transform = Compose([LetterBox(new_shape=(imgsz, imgsz),scaleup=False)])
    flip_idx = dataset.data.get("flip_idx", [])  # for keypoints augmentation
    if dataset.use_keypoints:
        kpt_shape = dataset.data.get("kpt_shape", None)
        if len(flip_idx) == 0 and hyp.fliplr > 0.0:
